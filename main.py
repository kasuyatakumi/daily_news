...
def analyze_news(news_list):
    ...
    prompt = f"""
あなたは人材紹介業（転職エージェント）の優秀なマネージャーです。
以下の直近24時間のニュース記事リストから、キャリアアドバイザー（CA）やリクルーティングアドバイザー（RA）が「求職者面談」や「企業開拓（テレアポ等）」の営業トークで **一番使える・刺さる記事を1つ** だけ選定してください。

【ニュース記事リスト】
{news_text}

選定した記事について、以下のフォーマットで出力してください。Markdown等の装飾は使わず、プレーンテキストに近い形で出力してください。
...
[記事の要約3行目]
4.営業トークへの活用例:
[なぜこのニュースが営業に使えるのか、どう求職者や企業に話せばいいのかの具体的な営業トークの切り口]
"""
